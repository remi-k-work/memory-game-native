...
const bounce = (value: SharedValue<number>, color: SharedValue<string>, delta: number, direction: number, bounds: [number, number]) => {
  "worklet";
  value.value = clamp(value.value + velocity * delta * direction, bounds[0], bounds[1]);
  if (value.value === bounds[0] || value.value === bounds[1]) {
    direction *= -1;
    color.value = colors.filter((c) => c !== color.value)[Math.floor(Math.random() * (colors.length - 1))];
  }
  return direction;
};
...
const animationGenerator: AnimationGenerator<typeof animationInitState> = function* ({ x, y, color }) {
  ...
  while (true) {
    const delta = yield* timeSincePrevFrame();
