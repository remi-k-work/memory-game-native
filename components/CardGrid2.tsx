...
// components
import SingleCard from "@/components/single-card/index2";
...
export default function CardGrid({ cols, rows, onCardImageLoaded }: CardGridProps) {
      ...
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <View key={rowIndex} className="flex-1 flex-row gap-1">
          {Array.from({ length: cols }).map((_, colIndex) => (
            <SingleCard
              key={currentCards[rowIndex * cols + colIndex].uniqueId}
              card={currentCards[rowIndex * cols + colIndex]}
              onCardImageLoaded={onCardImageLoaded}
            />
          ))}
        </View>
      ))}
