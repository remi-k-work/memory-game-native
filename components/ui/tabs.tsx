...
function TabsList({
  ...
  return (
    <TabsPrimitive.List
      className={cn("native:h-12 native:px-1.5 h-10 items-center justify-center rounded-md bg-muted p-1 web:inline-flex", className)}
      {...props}
    />
  );
}
...
function TabsTrigger({
  ...
  return (
    <TextClassContext.Provider
      value={cn("text-sm native:text-base font-medium text-muted-foreground web:transition-all", value === props.value && "text-foreground")}
    >
